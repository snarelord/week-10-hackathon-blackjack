...
const GoFish = () => {
  ...
  const instructions = [
    "Welcome to Go Fish! The goal is to collect the most 'books' of cards. A 'book' is a set of four cards with the same value (e.g., four Kings).",
    "On your turn, ask the computer for a card value you have in your hand. If the computer has the card, you get all of their cards of that value. If the computer doesn't have the card, you 'Go Fish' and draw from the deck.",
    "If you get the card you asked for from the deck, you get another turn. The game ends when all books are collected. The player with the most books wins!",
    "Click on a card in your hand to select it, then click 'Ask for Card' to make your move. Ready to play? Click 'Start Game' when you're done reading the instructions!",
  ];
  ...
  const dealInitialHands = (deck: CardProps[]) => {
    ...
    setMessage(
      "Cards dealt. Select a card from your hand and click 'Ask for Card'."
    );
  };
  ...
  const askForCard = () => {
    ...
    setTimeout(() => {
      ...
      if (matchingCards.length > 0) {
        ...
        setMessage(
          `You got ${matchingCards.length} ${value}${
        ...
        setMessage(
          "Go Fish! Click the 'Go Fish' button to draw a card from the deck."
        );
      }
  ...
  const goFish = (player: "player" | "computer") => {
    ...
    if (player === "player") {
      ...
      setTimeout(() => {
        setMessage(
          "It's your turn again. Select a card and click 'Ask for Card'."
        );
      }, 3000);
    }
  };
  ...
  const computerTurn = () => {
    ...
    const randomValue =
      computerHand[Math.floor(Math.random() * computerHand.length)].value;
    ...
    setTimeout(() => {
      ...
      if (matchingCards.length > 0) {
        ...
        setMessage(
          `You gave ${matchingCards.length} ${randomValue}${
            matchingCards.length > 1 ? "s" : ""
          } to the computer.`
        );
        checkAndUpdateBooks(newComputerHand, false);
  ...
  const checkGameOver = () => {
    ...
    if (
      ...
      if (playerBooks.length > computerBooks.length) {
        setMessage(
          `Game Over! You win with ${playerBooks.length} books to ${computerBooks.length}!`
        );
      } else if (computerBooks.length > playerBooks.length) {
        setMessage(
          `Game Over! Computer wins with ${computerBooks.length} books to ${playerBooks.length}!`
        );
      } else {
        setMessage(
          `Game Over! It's a tie with ${playerBooks.length} books each!`
        );
      }
    }
  };
  ...
  const removeBookCards = (hand: CardProps[], books: string[]): CardProps[] => {
  ...
  const checkAndUpdateBooks = (hand: CardProps[], isPlayer: boolean) => {
    ...
    if (books.length > 0) {
      if (isPlayer) {
        setPlayerBooks((prevBooks) => [...prevBooks, ...books]);
        setPlayerHand((prevHand) => removeBookCards(prevHand, books));
      } else {
        setComputerBooks((prevBooks) => [...prevBooks, ...books]);
        setComputerHand((prevHand) => removeBookCards(prevHand, books));
      }

      setMessage(
        `${isPlayer ? "You" : "Computer"} completed ${books.length} book${
          books.length > 1 ? "s" : ""
        }! (${books.join(", ")})`
      );
    }
        ...
        <div className={styles.instructions}>
          ...
          <div className={styles.instructionButtons}>
            <button
              ...
              onClick={() =>
                setCurrentInstruction(Math.max(0, currentInstruction - 1))
              }
